...
func main() {
	...
	sub, err := nc.JS.PullSubscribe(events.UsageSubject, "billing-workers", nats.BindStream(cfg.NATSStream))
	...
	for {
		...
		for _, msg := range msgs {
			...
			cost := pricing.Cost(event.Units, pricing.Default)
			logger.Info("usage billed",
				zap.String("event_id", event.EventID),
				zap.String("owner_id", event.OwnerID),
				zap.Int64("units", event.Units),
				zap.Int64("cost_minor", cost))
