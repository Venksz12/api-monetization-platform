...
import (
	...
	"github.com/yourusername/api-monetization-platform/services/billing"
)
...
func main() {
	...
	worker := &billing.Worker{
		Messaging: nc, Usage: &repository.UsageRepository{CB: cb},
		Invoices: &repository.InvoiceRepository{CB: cb}, Metrics: metrics,
	}
	go func() {
	...
	srv := &http.Server{Addr: cfg.BillingMetricsAddr, Handler: http.DefaultServeMux}
